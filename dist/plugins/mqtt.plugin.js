 ...
 */
const buildConnect = (clientId, username, password) => {
    ...
    if (username)
        payload = Buffer.concat([payload, encodeString(username)]);
    if (password)
        payload = Buffer.concat([payload, encodeString(password)]);
...
//# sourceMappingURL=mqtt.plugin.js.map