...
const plugin: Plugin = {
  ...
  description:
    "Erstellt einen neuen Termin in Google Kalender zu einem bestimmten Datum und einer bestimmten Uhrzeit.",
  examplePattern:
    "^Erstelle einen Termin (?<title>.+) am (?<date>\\d{4}-\\d{2}-\\d{2}) um (?<time>\\d{1,2}:\\d{2})$",
  handle: async function (
    ...
    await hooks.log(
      `Calendar event intent sent: ${title} on ${formattedDate} at ${time}`,
    );
    return {
